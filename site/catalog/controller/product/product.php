	...
	public function index() {
		...
		if ($product_info) {
			...
			foreach ($data['breadcrumbs'] as $i => $crumb) {
				$schema_bc['itemListElement'][] = array('@type' => 'ListItem', 'position' => $i + 1, 'name' => strip_tags($crumb['text']), 'item' => $crumb['href']);
			}
			...
			$schema_p = array(
				...
				'sku' => $product_info['sku'], 'mpn' => $product_info['model'], 'model' => $product_info['model'],
				'brand' => $product_info['manufacturer'] ? array('@type' => 'Brand', 'name' => $product_info['manufacturer']) : null,
				'offers' => array(
					'@type' => 'Offer',
			...
			$schema_html .= '<script type="application/ld+json">' . json_encode($schema_p, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) . '</script>';
