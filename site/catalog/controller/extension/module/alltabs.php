...
class ControllerExtensionModuleAlltabs extends Controller {
	public function index($setting) {
		...
		static $module = 0;
		$this->load->language('extension/module/alltabs');
		...
		$latest_results = $this->model_catalog_product->getLatestProducts($setting['limit']);
			...
			$data['all_prod_latest'] = $this->model_ns_newstore->multi_implode(",", $latest_results);
			...
			$this->load->model('ns/newstore');
			$data['all_prod_special'] = $this->model_ns_newstore->multi_implode(",", $special_results);
			...
			$this->load->model('ns/newstore');
			$data['all_prod_bestseller'] = $this->model_ns_newstore->multi_implode(",", $bestseller_results);
		...
		$this->load->model('ns/newstore');
		$data['all_prod_popular'] = $this->model_ns_newstore->multi_implode(",", $results);
		...
		if($products){	
		$this->load->model('ns/newstore');
			$data['all_prod_featured'] = $this->model_ns_newstore->multi_implode(",", $products);
