	...
	public function index() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['action'])) {
			...
			if ($this->validate()) {
				...
				if ($this->config->get('config_telegram_alert')) {
					$this->load->model('checkout/order');
					$order_info = $this->model_checkout_order->getOrder($results);
					$products = array();
					
					$order_products = $this->model_checkout_order->getOrderProducts($results);
					...
					$message .= "_Быстрый заказ № " . $order_info['order_id'] . " от " . date($this->language->get('datetime_format'), strtotime($order_info['date_added'])) . "_" . "\n";
					$message .= "Имя: " . $order_info['firstname'] . "\n";
					$message .= "Телефон: " . $order_info['telephone'] . "\n";
					$message .= "Сумма: " . round($order_info['total'],0) . "\n" . "\n";
					
					$message .= "_Состав заказа:_" . "\n";
					foreach ($products as $product) {
						$message .= $product['name'] . " - " . $product['quantity'] . " шт." . "\n";
					...
					$this->load->model('extension/module/telegram');
					$this->model_extension_module_telegram->SendMessage($message);
				}
