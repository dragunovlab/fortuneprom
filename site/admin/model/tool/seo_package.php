...
class ModelToolSeoPackage extends Model {
  ...
  public function getManufacturerData($manufacturer_id, $lang, $store_id) {
    ...
    if ($values[1] == 'upcase') {
      return mb_convert_case($values[2], MB_CASE_UPPER, 'UTF-8');
    } else if ($values[1] == 'lowcase') {
      return mb_convert_case($values[2], MB_CASE_LOWER, 'UTF-8');
    } else if ($values[1] == 'ucfirst') {
      return mb_convert_case($values[2], MB_CASE_TITLE, 'UTF-8');
    }
  }
    ...
    */
    ...
    if (strpos($pattern, '[name_') !== false) {
      ...
      foreach ($langDescriptions as $langDesc) {
        $replace['[name_'.$langDesc['language_id'].']'] = $langDesc['name'];
      }
    }
    ...
    if (strpos($pattern, '[desc]') !== false) {
      if (function_exists('mb_substr')) {
       ...
       preg_match('#(.*?(?:<\/p>|\.|\!|\?))#is', strip_tags(html_entity_decode($row['description'], ENT_QUOTES, 'UTF-8')), $desc_extract);
    ...
    if (strpos($pattern, '[desc_sentence2]') !== false) {
       preg_match('#.*?(?:<\/p>|\.|\!|\?)(.*?(?:<\/p>|\.|\!|\?))#is', strip_tags(html_entity_decode($row['description'], ENT_QUOTES, 'UTF-8')), $desc_extract);
       
       if (isset($desc_extract[1])) {
         $replace['[desc_sentence2]'] = strip_tags($desc_extract[1]);
       } else {
         $replace['[desc_sentence2]'] = '';
       }
    }
    if (strpos($pattern, '[desc_sentence3]') !== false) {
       preg_match('#.*?(?:<\/p>|\.|\!|\?).*?(?:<\/p>|\.|\!|\?)(.*?(?:<\/p>|\.|\!|\?))#is', strip_tags(html_entity_decode($row['description'], ENT_QUOTES, 'UTF-8')), $desc_extract);
       
       if (isset($desc_extract[1])) {
         $replace['[desc_sentence3]'] = strip_tags($desc_extract[1]);
       } else {
         $replace['[desc_sentence3]'] = '';
       }
    }
    if (strpos($pattern, '[desc_sentence4]') !== false) {
       preg_match('#.*?(?:<\/p>|\.|\!|\?).*?(?:<\/p>|\.|\!|\?).*?(?:<\/p>|\.|\!|\?)(.*?(?:<\/p>|\.|\!|\?))#is', strip_tags(html_entity_decode($row['description'], ENT_QUOTES, 'UTF-8')), $desc_extract);
       
       if (isset($desc_extract[1])) {
         $replace['[desc_sentence4]'] = strip_tags($desc_extract[1]);
       } else {
         $replace['[desc_sentence4]'] = '';
       }
    }
    ...
    if (!isset($row['description'])) {
      if (isset($row['orig_description'])) {
		...
		if (strpos($pattern, '[name]') !== false)
			$replace['[name]'] = trim($row['name']);
    if (strpos($pattern, '[name_') !== false) {
      $langDescriptions = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_description WHERE category_id = ".(int) $row['category_id'])->rows;
    ...
    if (strpos($pattern, '[grandparent]') !== false) {
      if (!empty($row['category_id'])) {
        $replace['[grandparent]'] = trim($this->getGrandParentCategoryName($row['category_id'], $lang));
      } else if (!empty($row['parent_id'])) {
        $replace['[grandparent]'] = trim($this->getParentCategoryName($row['parent_id'], $lang));
      } else {
        $replace['[grandparent]'] = '';
      }
    }
    if (strpos($pattern, '[parent]') !== false) {
      if (!empty($row['category_id'])) {
        $replace['[parent]'] = trim($this->getParentCategoryName($row['category_id'], $lang));
      } else if (!empty($row['parent_id'])) {
        $replace['[parent]'] = trim($this->getCategoryName($row['parent_id'], $lang));
      } else {
        $replace['[parent]'] = '';
      }
    }
    ...
    if (strpos($pattern, '[name_') !== false) {
      $langDescriptions = $this->db->query("SELECT * FROM " . DB_PREFIX . "information_description WHERE information_id = ".(int) $row['information_id'])->rows;
      
      foreach ($langDescriptions as $langDesc) {
        $replace['[name_'.$langDesc['language_id'].']'] = $langDesc['title'];
      }
    }
