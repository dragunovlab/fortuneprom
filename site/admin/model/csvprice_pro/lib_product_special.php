...
class ModelCSVPriceProLibProductSpecial extends Model
{
    ...
    public function getProductSpecial($product_id)
    {
        ...
        if (isset($CSVPRICEPRO_CORE['BASE_PRICE_SPECIAL']) && $CSVPRICEPRO_CORE['BASE_PRICE_SPECIAL']) {
            $query = $this->db->query('SELECT CONCAT( ps.customer_group_id, \',\', ps.priority, \',\', TRUNCATE(ps.price, 2), \',\', TRUNCATE(ps.base_price, 2), \',\', ps.date_start, \',\', ps.date_end) AS p_special FROM `' . DB_PREFIX . 'product_special` ps WHERE ps.product_id = ' . (int)$product_id);
        } else {
    ...
    public function addProductSpecial($product_id, $special)
    {
        ...
        if (!empty($special_data)) {
            foreach ($special_data as $product_special) {
                ...
                if (isset($CSVPRICEPRO_CORE['BASE_PRICE_SPECIAL']) && $CSVPRICEPRO_CORE['BASE_PRICE_SPECIAL'] && count($product_special) == 6) {
                    $this->db->query('INSERT INTO `' . DB_PREFIX . 'product_special`
                        SET product_id = \'' . (int)$product_id . '\', 
                            ...
                            base_price = \'' . $this->model_csvprice_pro_app_product->validateNumberFloat($product_special[3]) . '\',
                            date_start = \'' . ((isset($product_special[4])) ? $this->db->escape($product_special[4]) : '') . '\', 
                            date_end = \'' . ((isset($product_special[5])) ? $this->db->escape($product_special[5]) : '') . '\'');
                } else {
                    $this->db->query('INSERT INTO `' . DB_PREFIX . 'product_special`
			    SET product_id = \'' . (int)$product_id . '\', 
                            customer_group_id = \'' . (int)$product_special[0] . '\',
