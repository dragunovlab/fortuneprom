...
class ModelCSVPriceProLibProductFilter extends Model
{
    ...
    public function getProductFilters($product_id)
    {
        ...
        foreach ($query->rows as $result) {
            $filters[] = html_entity_decode($result['p_filters'], ENT_QUOTES, 'UTF-8');
        }
    ...
    public function getCategoryFilters($catgeory_id)
    {
        ...
        foreach ($query->rows as $result) {
            $filters[] = html_entity_decode($result['c_filters'], ENT_QUOTES, 'UTF-8');
        }
    ...
    private function getFilterIdByName($filter_name, $group_name)
    {
        ...
        $result = $this->db->query('SELECT filter_id FROM `' . DB_PREFIX . 'filter_description` WHERE LOWER(name) = LOWER(\'' . $this->db->escape(htmlspecialchars(trim($filter_name), ENT_QUOTES, 'UTF-8')) . '\') AND language_id = \'' . (int)$this->LanguageID . '\' AND filter_group_id = \'' . $filter_group_id . '\' LIMIT 1');
    ...
    {
        $result = $this->db->query('SELECT filter_group_id FROM `' . DB_PREFIX . 'filter_group_description` WHERE LOWER(name) = LOWER(\'' . $this->db->escape(htmlspecialchars(trim($group_name), ENT_QUOTES, 'UTF-8')) . '\') AND language_id = \'' . (int)$this->LanguageID . '\' LIMIT 1');
    ...
    private function addFilterGroup($group_name, $sort_order = 0)
    {
        ...
        $this->db->query('INSERT INTO `' . DB_PREFIX . 'filter_group_description` SET language_id = \'' . (int)$this->LanguageID . '\', filter_group_id = \'' . $filter_group_id . '\',  name = \'' . $this->db->escape(htmlspecialchars($group_name, ENT_QUOTES, 'UTF-8')) . '\'');
    ...
    private function addFilter($filter_group_id, $filter_name, $sort_order = 0)
    {
        ...
        $this->db->query('INSERT INTO `' . DB_PREFIX . 'filter_description` SET language_id = \'' . (int)$this->LanguageID . '\', filter_id = \'' . $filter_id . '\', filter_group_id = \'' . $filter_group_id . '\',  name = \'' . $this->db->escape(htmlspecialchars($filter_name, ENT_QUOTES, 'UTF-8')) . '\'');
