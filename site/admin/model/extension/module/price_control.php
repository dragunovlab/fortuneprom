        ...
        if (!empty($price_types)) {
            ...
            foreach ($price_types as $price_type) {
                ...
                switch ($price_type) {
                    ...
                    case self::PRICE_CONTROL_TYPE_DISCOUNTS:
                        ...
                        if (!empty($filter['priceControl_create_if_not_exists'])) {
                            ...
                            $allprod = $this->db->query("SELECT DISTINCT a.product_id as product_id,a.price as price FROM `" . DB_PREFIX . "product` a  LEFT JOIN `" . DB_PREFIX . "product_discount` disc ON disc.product_id=a.product_id " . (!empty($filter['categories']) ? " LEFT JOIN `" . DB_PREFIX . "product_to_category` cat ON cat.product_id=a.product_id" : "") . " WHERE ". ($where_filter ? $where_filter : '1') ." GROUP BY a.product_id");
                            if ($allprod->num_rows) {
                                foreach ($allprod->rows as $allprod_row) {
                                    foreach ($customer_groups as $id) {
                                        $this->db->query("DELETE FROM `" . DB_PREFIX . "product_discount` WHERE product_id=".(int)$allprod_row['product_id']." AND quantity='" . (int)$filter['new_discount_quantity'] . "' AND customer_group_id=".(int)$id);
                                        $this->db->query("INSERT INTO `" . DB_PREFIX . "product_discount`(`product_id`,`quantity`,`customer_group_id`,`price`) VALUES('" . (int)$allprod_row['product_id'] . "','" . (int)$filter['new_discount_quantity'] . "','" . (int)$id . "','" . $allprod_row['price'] . "')");
                                        $new_discounts[] = $this->db->getLastId();
                    ...
                    case self::PRICE_CONTROL_TYPE_ACTIONS:
                        ...
                        if (!empty($filter['priceControl_create_if_not_exists'])) {
                            ...
                            if ($allprod->num_rows) {
                                foreach ($allprod->rows as $allprod_row) {
                                    foreach ($customer_groups as $id) {
                                        $this->db->query("DELETE FROM `" . DB_PREFIX . "product_special` WHERE product_id=".(int)$allprod_row['product_id']." AND customer_group_id=".(int)$id);
                                        $this->db->query("DELETE FROM `" . DB_PREFIX . "pricecontrol_data` WHERE product_id=".(int)$allprod_row['product_id']." AND type=".(int)$price_type);
                                        $this->db->query("INSERT INTO `" . DB_PREFIX . "product_special`(`product_id`,`customer_group_id`,`price`) VALUES('" . (int)$allprod_row['product_id'] . "','" . (int)$id . "','" . $allprod_row['price'] . "')");
                                        $new_specials[] = $this->db->getLastId();
                ...
                switch ($price_type) {
                    ...
                    case self::PRICE_CONTROL_TYPE_ACTIONS:
                        foreach ($customer_groups as $id) {
                            $this->db->query("UPDATE `" . DB_PREFIX . "product_special` a, `" . DB_PREFIX . "pricecontrol_data` b SET a.price=b.price_edited WHERE a.product_id=b.product_id AND  b.type='" . (int)$price_type . "' AND a.customer_group_id=". $id);
                        }
                        break;
                }
            }
