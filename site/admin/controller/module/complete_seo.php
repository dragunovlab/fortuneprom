...
class ControllerModuleCompleteSeo extends Controller {
  ...
  public function index() {
    ...
    */
    ...
    $gkdMSLang = $this->config->get('config_gkdMSLang');
    
    if (!empty($gkdMSLang)) {
      foreach($languages as $k => $lang) {
        if (!empty($gkdMSLang[$lang['code']])) {
          unset($languages[$k]);
        }
      }
        ...
        */
        ...
        if ($mode == 'image_name')
        {
          ...
          foreach ($additionalImages as $imgIndex => $addImageRow) {
            ...
            $subValue = $this->model_tool_seo_package->transformProduct($subValue, $lang, $row, $this->store);
        
            $filename = $this->model_tool_seo_package->filter_seo($subValue, 'image', '', $lang, false, true);
